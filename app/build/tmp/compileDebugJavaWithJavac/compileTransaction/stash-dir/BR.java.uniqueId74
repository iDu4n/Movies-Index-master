package androidx.databinding.library.baseAdapters;

public class BR {
  public static final int _all = 0;

  public static final int adult = 1;

  public static final int backdropPath = 2;

  public static final int budget = 3;

  public static final int cast = 4;

  public static final int castsList = 5;

  public static final int checkBox = 6;

  public static final int contentMovie = 7;

  public static final int gender = 8;

  public static final int genres = 9;

  public static final int id = 10;

  public static final int imdbId = 11;

  public static final int locale = 12;

  public static final int movie = 13;

  public static final int originalLanguage = 14;

  public static final int originalTitle = 15;

  public static final int overview = 16;

  public static final int popularity = 17;

  public static final int posterPath = 18;

  public static final int releaseDate = 19;

  public static final int revenue = 20;

  public static final int review = 21;

  public static final int reviewsList = 22;

  public static final int runtime = 23;

  public static final int status = 24;

  public static final int stringBuilder = 25;

  public static final int tagline = 26;

  public static final int title = 27;

  public static final int torrent = 28;

  public static final int video = 29;

  public static final int voteAverage = 30;
}
