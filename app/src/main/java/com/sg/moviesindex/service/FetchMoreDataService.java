...
public class FetchMoreDataService {
    ...
    public void loadMore(int a, final int pages) {
        ...
        if (a == 0) {
            ...
            observableMovie = TMDbService.getUpcomingMoviesWithRx(ApiKey, pages, MainPage.region);
        } else if (a == 1) {
            observableMovie = TMDbService.getNowPlayingWithRx(ApiKey, pages, MainPage.region);
        }
    ...
    public void loadMoreGenres(final int pages) {
        final TMDbService TMDbService = RetrofitInstance.getTMDbService(context);
        String ApiKey = BuildConfigs.apiKey;
        Observable<DiscoversList> observableDB = TMDbService.discover(ApiKey, Long.toString(MainPage.genreid), false, false, pages, "popularity.desc", null, null, null, null);
