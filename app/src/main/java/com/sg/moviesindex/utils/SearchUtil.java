    ...
    public void search(final SearchView searchView) {
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
                            ...
                            .subscribeWith(new DisposableObserver<DiscoversList>() {
                                ...
                                public void onNext(@NotNull DiscoversList discoversList) {
                                    if (discoversList != null && discoversList.getResults() != null) {
                                        MainPage.discovers = (ArrayList<Discover>) discoversList.getResults();
                                        MainPage.totalPages = discoversList.getTotalPages();
                                        DiscoverToMovie discoverToMovie = new DiscoverToMovie(MainPage.discovers);
                                        MainPage.movieList = discoverToMovie.getMovies();
                                        if (progressBar != null) {
            ...
            public boolean onQueryTextChange(String newText) {
                final TMDbService TMDbService = RetrofitInstance.getTMDbService(context);
                final String ApiKey = BuildConfigs.apiKey;
                MainPage.queryM = newText;
                MainPage.drawer = 6;
                observableDB = TMDbService.search(ApiKey, false, MainPage.queryM, 1);
                compositeDisposable.add(observableDB
                        ...
                        .subscribeWith(new DisposableObserver<DiscoversList>() {
                            ...
                            public void onNext(DiscoversList discoversList) {
                                if (discoversList != null && discoversList.getResults() != null) {
                                    MainPage.search = (ArrayList<Discover>) discoversList.getResults();
                                    DiscoverToMovie discoverToMovie = new DiscoverToMovie(MainPage.search);
                                    MainPage.moviesearch = discoverToMovie.getMovies();
                                    String[] a = new String[MainPage.moviesearch.size()];
                                    for (int i = 0; i < a.length; i++) {
                                        a[i] = MainPage.moviesearch.get(i).getTitle();
                                    }
                                    ...
                                    SearchAdapter searchAdapter = new SearchAdapter(context, cursor, true, MainPage.moviesearch);
                ...
                .subscribeWith(new DisposableObserver<DiscoversList>() {
                    ...
                    public void onNext(DiscoversList discoversList) {
                        if (discoversList != null && discoversList.getResults() != null) {
                            ...
                            MainPage.totalPages = discoversList.getTotalPages();
                            DiscoverToMovie discoverToMovie = new DiscoverToMovie(MainPage.discovers);
                            MainPage.movieList.addAll(discoverToMovie.getMovies());
                        }
                    }
